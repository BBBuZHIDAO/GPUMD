        ...
        for (int ib = 0; ib < num_cell[1]; ++ib) {
          for (int ic = 0; ic < num_cell[2]; ++ic) {
            ...
            if (distance_square < rc_angular * rc_angular) {
              ...
              count_angular++;
            }else if (distance_square < rc_radial * rc_radial) {
...
static __global__ void find_descriptors_radial_and_angular(
  const int N,
  ...
  float* g_sum_fxyz)
{
  ...
  if (n1 < N) {
    ...
    float q_angular[MAX_DIM_ANGULAR] = {0.0f};
    ...
    for (int n = 0; n <= paramb.n_max_angular; ++n) {
      if (paramb.num_L == paramb.L_max) {
        ...
        find_q_with_4body(paramb.n_max_angular + 1, n, s, q_angular);
      } else {
        find_q_with_5body(paramb.n_max_angular + 1, n, s, q_angular);
      }
...
static __global__ void find_force_radial_and_angular(
  ...
  float* g_virial)
{
  ...
  if (n1 < N) {
    ...
    for (int i1 = 0; i1 < neighbor_number_angular; ++i1) {
      ...
      atomicAdd(&g_fx[n2_angular], -f12[0]);
      atomicAdd(&g_fy[n2_angular], -f12[1]);
      atomicAdd(&g_fz[n2_angular], -f12[2]);
    ...
    for (int i1 = 0; i1 < neighbor_number_radial; ++i1) {
      int index = i1 * N + n1;
      int n2_radial = g_NL_radial[index];
      int t2 = g_type[n2_radial];
      ...
      atomicAdd(&g_fy[n1], f12[1]);
      atomicAdd(&g_fz[n1], f12[2]);
      atomicAdd(&g_fx[n2_radial], -f12[0]);
      atomicAdd(&g_fy[n2_radial], -f12[1]);
      atomicAdd(&g_fz[n2_radial], -f12[2]);
  ...
  int device_in_this_iter)
{
  ...
  for (int device_id = 0; device_id < device_in_this_iter; ++device_id) {
    ...
    find_descriptors_radial_and_angular<<<grid_size, block_size>>>(
      dataset[device_id].N,
      nep_data[device_id].NN_radial.data(),
      nep_data[device_id].NL_radial.data(),
      nep_data[device_id].NN_angular.data(),
    ...
    find_force_radial_and_angular<<<grid_size, block_size>>>(
      is_dipole,
      dataset[device_id].N,
      nep_data[device_id].NN_radial.data(),
      nep_data[device_id].NL_radial.data(),
